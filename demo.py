...
st.set_page_config(
    page_title="Fashion Price Predictor",
    page_icon="🛍️",
    layout="wide",
...
def load_models():
    try:
        with open('random_forest_model_regpkl.pkl', 'rb') as file:
            reg_model = pickle.load(file)
        with open('logistic_reg_clss.pkl', 'rb') as file:
            clf_model = pickle.load(file)
        with open('scalerfit_regression.pkl', 'rb') as file:
            scaler_reg = pickle.load(file)
        with open('scalerfit_classification.pkl', 'rb') as file:
...
# Feature definitions
reg_features = ['page2_clothing_model', 'page1_main_category', 'model_photography']
